...
function vowelCount(str){
   ...
   strToArr.forEach(function(val) {
    ...
    if (vowels.indexOf(lowercased) !== -1) {
        // Checks to see if a key for the vowel has been created
        if (vowelObjs[lowercased]) {
            vowelObjs[lowercased]++;
        }
        else{
            vowelObjs[lowercased] = 1;
        }
    }
   });
   return vowelObjs;
...
function find(arr, searchValue) {
    const ele = arr.filter(function(element){
        return element === searchValue;
    })[0]; // We only want one element returned
    return ele;
}
...
function findInObj(arr, key, searchValue) {
    const newArr = arr.filter(function(person){
        return person[key] !== null && person[key] == searchValue;
    })[0]; // We only want one element returned
    return newArr;
}
